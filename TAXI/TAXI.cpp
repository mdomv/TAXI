...
float zysk_na_godzine(float,float);
int main()
{
	float koszty_stale;
	...
	pokaz_wariant("Xsara: ", paliwo1, spalanie1, ilosc_kursow1, kwota_za_kurs3, koszty_stale, ilosc_godzin1, ilosc_godzin2, ilosc_godzin3, kurs);
	...
	pokaz_wariant("Corolla: ",corolla_paliwo,corolla_spalanie,ilosc_kursow1,kwota_za_kurs3,koszty_stale,ilosc_godzin1,ilosc_godzin2, ilosc_godzin3,kurs);
	//cout << "Dupa" << endl;
	return 0;
}
...
void pokaz_wariant(const char* opis, float paliwo, float spalanie, float ilosc_kursow, float kwota_za_kurs, float koszty_stale, float ilosc_godzin1, float ilosc_godzin2, float ilosc_godzin3, float kurs) {
	...
	float zarobek2 = 0;
	float zarobek3 = 0;
	zarobek1 = zysk_na_godzine(zysk_netto(ilosc_godzin1, ilosc_kursow, kwota_za_kurs, koszty_stale, paliwo, spalanie, kurs), ilosc_godzin1);
	zarobek2 = zysk_na_godzine(zysk_netto(ilosc_godzin2, ilosc_kursow, kwota_za_kurs, koszty_stale, paliwo, spalanie, kurs), ilosc_godzin2);
	zarobek3 = zysk_na_godzine(zysk_netto(ilosc_godzin3, ilosc_kursow, kwota_za_kurs, koszty_stale, paliwo, spalanie, kurs), ilosc_godzin3);
	cout << opis << endl;
	...
	cout << "Zarobek na godzinę 40: " << zarobek1 << "zl 60: " << zarobek2 << "zl 600: " << zarobek3 << "zl" << endl;
...
int minimum_godzin(float ilosc_kursow, float kwota_za_kurs, float koszty_stale, float paliwo, float spalanie, float kurs) {
	float brutto = ilosc_kursow * kwota_za_kurs;
	float prowizja = brutto * prowizja_bolt;
	float zysk_na_godzine = 0;
	zysk_na_godzine = brutto - prowizja
		- koszt_na_kilometr(paliwo, spalanie) * kurs * ilosc_kursow;
...
float zysk_na_godzine(float zysk, float ilosc_godzin) {
	float zarobek = 0;
	zarobek = zysk / ilosc_godzin;
	return zarobek;
}